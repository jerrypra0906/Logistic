...
async function testFirstRow() {
  try {
    console.log('=== TESTING FIRST ROW ONLY ===');
    ...
    const processedResult = await pool.query(
      'SELECT contract_number, shipment_id, po_number, sto_number, supplier_name, product, vessel_name FROM sap_processed_data WHERE import_id = $1 LIMIT 1',
      [result.importId]
    );
    
    console.log('\n=== FIRST ROW PROCESSED DATA ===');
    if (processedResult.rows.length > 0) {
      const row = processedResult.rows[0];
      ...
      const hasData = row.contract_number || row.shipment_id || row.po_number || 
